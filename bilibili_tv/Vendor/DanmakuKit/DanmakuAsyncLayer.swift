...
public class DanmakuAsyncLayer: CALayer, @unchecked Sendable {
    ...
    public var didDisplay: (@Sendable (_ layer: DanmakuAsyncLayer, _ finished: Bool) -> Void)?
    ...
    private func display(isAsync: Bool) {
        ...
        if isAsync {
            ...
            let isCancelled: @Sendable () -> Bool = { () -> Bool in
            ...
            queue.async {
                ...
                if isCancelled() {
                    RunLoop.main.perform { self.didDisplay?(self, false) }
                    return
                }
                RunLoop.main.perform {
